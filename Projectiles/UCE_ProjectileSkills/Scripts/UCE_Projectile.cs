...
// UCE PROJECTILE
// =======================================================================================
public abstract class UCE_Projectile : SkillEffect
{
    [Header("-=-=-=- Settings -=-=-=-")]
    ...
    protected bool bArrivedAtTarget;
    ...
    protected void OnProjectileImpact(Entity _target = null)
    {
        ...
        if (data.createOnTarget.Length > 0 && data.createOnTarget.Length >= data.skillLevel - 1 && data.createOnTarget[data.skillLevel - 1] != null && Random.value <= data.createChance)
        {
    ...
    protected void RotateProjectile()
    {
       ...
       //     transform.RotateAround(transform.position, Vector3.forward, Time.deltaTime * rotationSpeed);
        ...
        if (lockDirection)
            ...
            if (data.sidekick)
            {
                float fDegrees = Random.Range(data.sidekickSpreadAngle * -1, Mathf.Abs(data.sidekickSpreadAngle));
                transform.Rotate(new Vector3(0, fDegrees, 0));
            }
           // else
          //  {
